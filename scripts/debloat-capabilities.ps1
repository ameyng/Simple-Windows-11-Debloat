...
# Debloat some capabilities based on their IDs
...
[String] $sBloatWareFilePath = "..\data\bloat_caps.list"
[String] $sCapSuffix = "~~~~0.0.1.0"
...
If ( Test-Path -Path "$sBloatWareFilePath" -PathType Leaf ) {
	...
	ForEach ( $sBloatID in $aBloatwareList ) {
		...
		# Set the complete name of the capability
		[String] $sCapabilityName = ${sBloatID} + ${sCapSuffix}
		
		Write-Host "${sCapabilityName}"
		
		# Disable the capability
		DISM.exe /Online /Remove-Capability /CapabilityName:"${sCapabilityName}" /NoRestart
	}
