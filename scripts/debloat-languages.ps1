...
# Debloat some languages based on their IDs
...
[String] $sBloatWareFilePath = "..\data\bloat_lang.list"
...
If ( Test-Path -Path "$sBloatWareFilePath" -PathType Leaf ) {
	...
	ForEach ( $sBloatID in $aBloatwareList ) {
		...
		DISM.exe /Online /Remove-Capability /CapabilityName:"${sBloatID}" /NoRestart
	}
